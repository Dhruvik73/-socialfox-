...
function Search({setToUser,setToUserDetails,setOldCHats,toUser}) {
  ...
  const submit = async () => {
    if (search) {
      ...
      const res = await fetch('http://65.0.19.137:5001/user/search', body)
  ...
  const follow = async (id) => {
    ...
    await fetch('http://65.0.19.137:5001/user/allies', mybody)
    const userbody = {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: id })
    }
    const myres = await fetch('http://65.0.19.137:5001/user/fetchuser', userbody)
    ...
    await fetch('http://65.0.19.137:5001/user/follow', b)
    setreload(reload + 1)
  }
  const unfollow = async (id) => {
    ...
    await fetch('http://65.0.19.137:5001/user/follower', b)
    setreload(reload + 1)
  }
