...
function Comment({ comments, bgColor, postId, setComments }) {
  ...
  const addComment = async () => {
    ...
    if (btn.innerText === "Post") {
      if (comment && comment.value.trim().length > 10) {
        ...
        await fetch('http://65.0.19.137:5001/post/addComment', body).then((res) => res.json()).then(async res => {
          if (!res.error) {
            const body = {
              method: "POST",
              headers: { 'Content-Type': 'application/json' },
            ...
            await fetch('http://65.0.19.137:5001/post/getComments', body).then((res) => res.json()).then((res) => { setComments(res.comments); comment.value = ""; })
  ...
  const addReply = async (commentId,reply) => {
    ...
    await fetch('http://65.0.19.137:5001/post/addReply', body).then((res)=>res.json()).then(async(res)=>{
      if(!res.error){
        ...
        await fetch('http://65.0.19.137:5001/post/getComments', body).then((res) => res.json()).then((res) => { setComments(res.comments);})
  ...
  const addLike = async (id,type) =>{
    ...
    await fetch('http://65.0.19.137:5001/post/commentlike', body).then((res)=>res.json()).then((res)=>{
     if(!res.error){
